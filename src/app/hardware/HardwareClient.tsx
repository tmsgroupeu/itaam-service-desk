...
import { createAsset, updateAsset, deleteAsset, assignAsset, unassignAsset, updateAssetStatus } from '@/app/actions'
          ...
          <div className="modal-body">
            <div className="form-grid-2">
              <div className="form-group">
                ...
                <select name="type" className="form-select" defaultValue={asset?.type ?? 'Serialized'} onChange={e => setType(e.target.value)}>
              ...
              <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                ...
                <textarea name="conditionComment" className="form-textarea" defaultValue={asset?.conditionComment ?? ''} placeholder="e.g. Keyboard replacement needed, extra charger included, Qty: 5" />
              </div>
            </div>
          </div>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <table className="data-table">
          ...
          <tbody>
            {filtered.map(a => (
              <tr key={a.id}>
                <td><span className={`badge ${a.type === 'Serialized' ? 'badge-blue' : 'badge-purple'}`}>{a.type}</span></td>
            ...
            {filtered.length === 0 && <tr><td colSpan={7} className="table-empty">No assets with status <strong>{filter}</strong>.</td></tr>}
          </tbody>
        </table>
      </div>
    </>
